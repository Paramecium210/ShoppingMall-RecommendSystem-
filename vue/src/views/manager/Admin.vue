...
export default {
  ...
  methods: {
    ...
    del(id) {   // 单个删除
      this.$confirm('Are you sure to delete？', 'Delete', {type: "warning"}).then(response => {
    ...
    delBatch() {   // 批量删除
      if (!this.ids.length) {
        this.$message.warning('Choose your data')
        return
      }
      this.$confirm('Are you sure to delete the data？', 'Delete', {type: "warning"}).then(response => {
